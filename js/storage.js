...
const Storage = {
    ...
    setWirdProgress: (progress) => Storage.set(Storage.KEYS.WIRD_PROGRESS, progress),
    setWirdProgress: (progress) => Storage.set(Storage.KEYS.WIRD_PROGRESS, progress),
    getWirdProgress: () => Storage.get(Storage.KEYS.WIRD_PROGRESS, { 
        day: 1, 
        completed: false, 
        lastDate: null,
        // We will default startDate to null here, logic will handle initialization
        startDate: null 
    }),

    // New Settings for Redistribute feature
    setWirdSettings: (settings) => Storage.set('ramadan_wird_settings', settings),
    getWirdSettings: () => Storage.get('ramadan_wird_settings', { 
